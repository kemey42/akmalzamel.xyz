 ...
 * 
 * Modified for akmalzamel.xyz
 * Use bootstrap and custom css altogether
 */
...
var GitHubActivity = (function() {
  ...
  var methods = {
    ...
    getMessageFor: function(data) {
      ...
      data.userGravatar = Mustache.render('<div class="gaf-gravatar-user"><img src="{{url}}" class="gaf-gravatar-small"></div>', { url: data.actor.avatar_url });
      ...
      if (p.commits) {
        ...
        p.commits.forEach(function(d, i) {
          ...
          if (i < 2) {
            d.shaLink = methods.renderGitHubLink(data.repo.name + '/commit/' + d.sha, d.sha.substring(0, 6), 'gaf-sha');
            d.committerGravatar = Mustache.render('<img class="gaf-gravatar-commit" src="https://gravatar.com/avatar/{{hash}}" width="16" />', { hash: md5(d.author.email.trim().toLowerCase()) });
          } else {
      ...
      if (p.pull_request) {
        ...
        if (p.pull_request.merged) {
          ...
          data.mergeMessage = Mustache.render('<br><small class="gaf-message-merge">' + message + '</small>', { c: pr.commits, a: pr.additions, d: pr.deletions });
        } else {
...
// Akmal: use bootstrap and custom template
var templates = {
  Stream: '<div class="gaf-feed list-group">{{{text}}}</div>',
  Activity: '<div id="{{id}}" class="gaf-activity list-group-item">\
               <div class="gaf-activity-icon"><span class="octicon octicon-{{icon}}"></span></div>\
               <div class="gaf-message"><div class="gaf-time">{{{timeString}}}</div>{{{userLink}}} {{{message}}}</div>\
             </div>',
  SingleLineActivity: '<div class="gaf-activity list-group-item">\
                         <div class="gaf-activity-icon"><span class="octicon octicon-{{icon}}"></span></div>\
                         <div class="gaf-message"><div class="gaf-time">{{{timeString}}}</div>{{{userLink}}} {{{message}}}</div>\
                       </div>',
  UserHeader: '<span class="d-none">\
                 {{withoutName}} {{{userNameLink}}} {{{userLink}}} {{{gravatarLink}}}\
               </span>',
  Footer: '<div><div/>',
  NoActivity: '<div class="list-group-item">This user does not have any recent public activity.</div>',
  UserNotFound: '<div class="list-group-item">User {{username}} wasn\'t found.</div>',
  EventsNotFound: '<div class="list-group-item">Events for user {{username}} not found.</div>',
  ...
  PushEvent: 'pushed to {{{branchLink}}}{{{repoLink}}}<br>\
                <ul class="gaf-commits">{{#payload.commits}}<li><small>{{{committerGravatar}}} {{{shaLink}}} {{message}}</small></li>{{/payload.commits}}</ul>\
                <small class="gaf-message-commits">{{{commitsMessage}}}</small>',
...
icons = {
  ...
  CreateEvent_repository: 'repo',
  CreateEvent_tag: 'tag',
  CreateEvent_branch: 'git-branch',
