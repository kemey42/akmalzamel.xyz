...
var GitHubActivity = (function () {
  ...
  var methods = {
    ...
    getMessageFor: function (data) {
      ...
      data.userGravatar = Mustache.render('<div class="gaf-gravatar-user"><img src="{{url}}" class="gaf-gravatar-small"></div>', {
      ...
      if (p.commits) {
        ...
        p.commits.forEach(function (d, i) {
          ...
          if (i < 2) {
            ...
            d.committerGravatar = Mustache.render('<img class="img-round" src="https://gravatar.com/avatar/{{hash}}" width="16" />', {
      ...
      if (p.pull_request) {
        ...
        if (p.pull_request.merged) {
          ...
          data.mergeMessage = Mustache.render('<br><small class="gaf-message-merge">' + message + '</small>', {
            c: pr.commits,
            a: pr.additions,
            d: pr.deletions
          });
...
var templates = {
    Stream: '<ul class="list-group">{{{text}}}</ul>',
    Activity: '<li class="list-group-item py-2">\
              <div class="row">\
               <div class="col-1 text-center"><span class="octicon octicon-{{icon}}"></span></div>\
               <div class="col-11"><div class="sml-text">{{{timeString}}}</div>{{{message}}}</div>\
              </div>\
               </li>',
    SingleLineActivity: '<li class="list-group-item py-2">\
                          <div class="row">\
                            <div class="col-1 text-center"><span class="octicon octicon-{{icon}}"></span></div>\
                            <div class="col-11"><div class="sml-text">{{{timeString}}}</div>{{{message}}}</div>\
                          </div>\
                         </li>',
    UserHeader: '<li class="d-none">\
                 {{withoutName}} {{{userNameLink}}} {{{userLink}}} {{{gravatarLink}}}\
               </li>',
    Footer: '<li class="d-none"><li/>',
    NoActivity: '<li class="list-group-item text-center">This user does not have any recent public activity.</li>',
    UserNotFound: '<li class="list-group-item text-center">User {{username}} wasn\'t found.</li>',
    EventsNotFound: '<li class="list-group-item text-center">Events for user {{username}} not found.</li>',
    ...
    PushEvent: 'Pushed to {{{branchLink}}}{{{repoLink}}}<br>\
                <ul class="commits">{{#payload.commits}}<li>{{{committerGravatar}}} <small>{{{shaLink}}} {{message}}</small></li>{{/payload.commits}}</ul>\
                <small class="sml-text">{{{commitsMessage}}}</small>',
